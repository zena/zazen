...
/* init */
void Init_zazen_ext() {
  rb_Zazen = rb_define_module("Zazen");
  rb_Parser = rb_define_class_under(rb_Zazen, "Parser", rb_cObject);
  ...
  /* classes */
  rb_SyntaxError = rb_const_get(rb_Zazen, rb_intern("SyntaxError"));
}
...
VALUE rb_parse(VALUE self, VALUE arg) {
  ...
  // raise Zazen::SyntaxError.new("Syntax error near #{data[p..-2].inspect}.") if p != pe
  if (p < pe) {
    p = p - 3;
    if (p < data) p = data;
    rb_raise(rb_SyntaxError, "Syntax error near %s.", RSTRING_PTR(rb_str_inspect(rb_str_new(p , pe - p - 1))));
